...
impl AptosDataClient {
        ...
        E: Into<Error>,
    {
        ...
        // Send the requests to the peers
        ...
        for _ in 0..num_sent_requests {
            let task_result = sent_requests.select_next_some().await;
            if let Ok(response_result) = task_result {
                match response_result {
                    Ok(response) => return Ok(response), // Return the response if it was successful
                    Err(error) => sent_request_errors.push(error),
                }
            }
        }
